...
program.option('-s, --serverUrl <type>', 'server address').option('-t, --token <type>', 'token')
/*
 *
 *  解析请求参数
 *
...
const { serverUrl, token } = program.opts()
if (!serverUrl) {
...
const scanner = async() => {
  const data = await readFileAsync(`${process.cwd()}/package.json`)
