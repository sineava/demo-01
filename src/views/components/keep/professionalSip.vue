...
<script>
import AddNewDialog from './newDialogShip'
import JyDialog from './jyDialogShip'
...
export default {
  ...
  methods: {
    initBusiness(data){
      ...
      if (data.shipManagePersonFilingDetailedInfoVoList && data.shipManagePersonFilingDetailedInfoVoList.length>0){
        data.shipManagePersonFilingDetailedInfoVoList.forEach(item=>{
          if (item.type===1){
    ...
    async submitForm() {
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          res.shipManagePersonFilingDetailedInfoDtoList = this.addTableData.length>0?this.addTableData:null
