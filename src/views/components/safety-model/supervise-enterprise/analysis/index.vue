<template>
  <el-card class="body-card">
    <div
      ...
      class="clearfix"
    >
      <span>重点监管企业风险研判分析</span>
      <div v-if="user_info.orgcategory==='1'" style="display: inline-block; float: right;">
        ...
        <div style="display: flex;">
          <el-card style="flex: 1;">
            ...
            <div style="line-height: 32px;">
              <div>分析指标：</div>
              <Charts style="height: 300px;" :option="barOption" />
            </div>
          </el-card>
        </div>
...
import { currentData, riskData, trendData, getList, del } from '@/api/analysis-model/enterprise-risk-analysis'
import { getList as getFirmList } from '@/api/analysis-model/firm-info'
...
export default {
  ...
  created() {
    ...
    if (this.user_info.orgcategory==='1') {
      ...
      this.showDetail(0, false)
    }
  },
  methods: {
    getLevelNameToColor,
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/supervise-enterprise/judge'})
    },
    // 查询风险等级列表
    async fetchLevel() {
      this.levels = (await getLevel(5)).data
    },
    async fetchFirmList() {
      // 辖区水上交通安全形式分析
      this.firmList = (await getFirmList()).data || []
      if (this.firmList.length>0){
        this.showDetail(this.firmList[0].id, this.firmList[0].isCounty === 0)
      }
    },
    // 重置查询条件
    resetForm() {
    ...
    async fetchData() {
      ...
      const res = await getList({
        ...
        firmInfoId: this.user_info.orgcategory==='1' ? this.id : null,
    ...
    handleDetail(item,isDetail) {
      this.$router.push({ path: '/safety-analysis-model/supervise-enterprise/judge', query: { id: item.id ,isDetail:isDetail}})
    },
    ...
    showDetail(id, total) {
      ...
      currentData(id).then(async res => {
        ...
        if (res.code === '0') {
          ...
          const option = res.data ? res.data.indicatorDataList : []
          let yData = []
          let xData = []
          option.forEach(item=>{
          ...
          if (this.totals) {
            const res = (await riskData()).data || []
            const arr = []
            res.forEach(x => {
              const value = x.controlLevelColour
              if (x.riskAreas && x.riskAreas.length>0){
                x.riskAreas.forEach(y => {
                  arr.push({ name: y.administrativeDivisionName, value })
                })
              }
            })
            this.mapOption = new MapChart([...arr]).option
          } else {
            this.mapOption = new MapChart([{ name: this.info.riskAreaName, value: ['高风险','较高风险','一般风险','较低风险','低风险'].indexOf(this.info.gradeName) + 1 }]).option
          }
          console.log(id)
