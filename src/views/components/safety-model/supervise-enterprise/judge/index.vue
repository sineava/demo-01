<template>
  <el-card class="body-card fx">
    <div
      ...
      class="clearfix"
    >
      <span>重点监管企业风险研判</span>
      <div style="display: inline-block; float: right;">
        <el-radio-group v-model="selected" @change="changeType">
...
import { getDataLevel } from '@/views/industry-server/safety-analysis-model/utils'
import { initData, assessData as assessInfo, completeData,areaInfo } from '@/api/analysis-model/enterprise-risk-analysis'
...
import Assessment from '@/views/industry-server/safety-analysis-model/components/enterprise'
...
export default {
  ...
  methods: {
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/supervise-enterprise/analysis'})
    },
    changeRoute(){
      this.init()
    ...
    async init(){
      ...
      this.enterprise = res.data && res.data.firmInfos ? res.data.firmInfos : []
      if (this.id){
    ...
    async assess() {
      ...
      this.assessData = (await startAssess(5)).data
      this.active = 2
    },
    // 提交评估
    async submitAssess() {
      ...
      const safeModeAppraisalAreaDto = {
        ...
        enterpriseId: this.assessmentData.enterpriseId,
        enterpriseName: this.assessmentData.enterpriseName,
        firmInfoId: this.assessmentData.firmInfoId,
        id: null,
        records: [...assess]
      }
