      ...
      this.searchData.forEach(element => {
        if (element.form==='daterange'){
          if (element.data && element.data.length>0){
            queryData[element.prop[0]] = momentDate(element.data[0],element.format[0].replace(/-dd/g, '-DD'))
            queryData[element.prop[1]] = momentDate(element.data[1],element.format[1].replace(/-dd/g, '-DD'))
          }
        } else {
          if (element.data!==null && element.data!=='' && element.data!==undefined){
            queryData[element.prop] =  element.data
          }
        }
      })
      return queryData
