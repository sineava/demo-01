  ...
  <el-card class="body-card">
    ...
    <div class="app-container">
      ...
      >
        ...
        <el-table-column label="审查状态" :show-overflow-tooltip="true">
          <template v-slot="scope">
            {{ scope.row.reviewState && reviewState.filter(item => (item.value === scope.row.reviewState))[0].label }}
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 详情
    handleDetail(item) {
      this.$router.push({ path: '/enterprise/goods/facilityInfo', query: { id: item.id }})
    }
  }
}
</script>
