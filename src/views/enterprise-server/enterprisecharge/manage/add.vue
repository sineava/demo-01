...
<script>
import { addStandard, queryDetail, queryDidNotMake } from '@/api/port-services/port-enterprise-charge-management/charge-enterprise'
...
export default {
  ...
  methods: {
    // 企业查询未制定的标准类型
    async fetchTableType() {
      queryDidNotMake().then(res => {
        if (!res.data) return
        this.options = res.data.map(({ tableName, tableType }) => ({ label: tableName, value: tableType }))
      })
    },
    // 标准类型改变
    handleChange(val) {
    ...
    async fetchDetail(tableName) {
      await queryDetail({ tableName }).then(res => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.tableName === 'SHIP_TUG') {
            chargeEnterpriseDto.chargeShipTugDtoList = this.$refs['shiptug'].getTableData().map(({
              id,
              parentId,
              ratePriceA,
              ratePriceB,
    ...
    async saveData(chargeEnterpriseDto) {
      this.loading = true
      addStandard(chargeEnterpriseDto).then(res => {
        this.loading = false
        if (res.code === '0') {
          this.returnBack()
        } else {
