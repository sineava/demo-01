    ...
    <div class="app-container">
      ...
      >
        ...
        <el-table-column
          ...
          label="操作"
          :width="(button_roles.desc ? 95 : 0) + (button_roles.handling ? 120 : 0)"
        >
          <template v-slot="scope">
            <el-button
              v-if="button_roles.desc"
...
<script>
import { pageEnList } from '@/api/port-services/record-management/service-en-filing'
...
export default {
  ...
  methods: {
    ...
    handleDetail(row, isEdit) {
      ...
      this.$router.push({ path: `/record/enterprise/acceptDetail?isEdit=${isEdit}&isAcceptance=${isAcceptance}&id=${row.id}` })
    },
    /**
     * 添加
     */
    add() {
      this.$router.push({ path: `/record/enterprise/addAccept` })
    }
  }
}
</script>
