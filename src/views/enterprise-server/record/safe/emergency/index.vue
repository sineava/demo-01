...
import { pageEnList } from '@/api/port-services/record-management/emergency-filing'
...
export default {
  ...
  methods: {
    ...
    handleDetail(row, isEdit) {
      ...
      this.$router.push({ path: `/record/safe/emergencyDetail?isEdit=${isEdit}&isAcceptance=${isAcceptance}&id=${row.id}` })
    },
    /**
     * 添加
     */
    add() {
      this.$router.push({ path: `/record/safe/addEmergency` })
    }
  }
}
</script>
