<template>
  <el-card class="body-card">
    ...
    <div class="basic-detail">
      <HiddenDanger ref="HiddenDanger" :data="ruleForm" :isEdit="isEdit" />
...
<script>
import { add, update, findById } from '@/api/port-services/record-management/major-threat-filing'
...
export default {
  ...
  methods: {
    ...
    async save() {
      const data = await this.$refs.HiddenDanger.submitForm()
      if(data===false){
        return false
      }
      this.loading = true
    ...
    // 返回列表
    returnBack() {
      this.$router.push({ path: `/record/safe/major` })
    }
  }
}
</script>
