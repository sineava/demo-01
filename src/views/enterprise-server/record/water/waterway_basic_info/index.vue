...
import { pageEnBaseFilingList } from '@/api/water-transportation-services/keep-on-record/wt-en-base-filing'
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (this.screenTime && this.screenTime.length > 0) {
        ...
        data.screenEndTime = this.screenTime[1] ? moment(this.screenTime[1]).format('YYYY-MM-DD 23:59:59') : ''
      }
      pageEnBaseFilingList(data).then(res => {
    ...
    handleDetail(item,oper) {
      this.$router.push({ path: '/record/water/waterway/basicInfoDetail', query: { id: item.id,oper:oper }})
    }
  }
}
</script>
