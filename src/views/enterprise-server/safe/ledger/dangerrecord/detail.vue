  ...
  <el-card class="body-card">
    ...
    >
      <span class="detail-back">
        <el-page-header @back="returnBack" content="企业自查隐患记录详情页" />
      </span>
    </div>
  ...
  methods: {
    initEdit(id) {
      getDetails(id).then(res => {
        ...
        this.tableData = [
          { ak: '检查项目', av: res.data.itemName, bk: '检查项分类', bv: res.data.categoryName, ck: '危险等级', cv: res.data.riskLevel === 1 ? '一般' : '严重' },
    ...
    // 返回
    returnBack() {
      this.$router.push({ path: '/safe/ledger/dangerrecord' })
    }
  }
}
</script>
