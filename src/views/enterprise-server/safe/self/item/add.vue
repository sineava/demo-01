  ...
  <el-card class="body-card">
    ...
    >
      <span class="detail-back">
        <el-page-header @back="returnBack" content="企业自查项" />
      </span>
    </div>
...
import { getItem, addExamineItem, updateExamineItem } from '@/api/general-services/enterprise-self-inspection/examineItem'
  ...
  methods: {
    ...
    initEdit() {
      getItem(this.ruleForm.id).then(res => {
        ...
        this.reviewedFile = JSON.parse(filePath || '[]')
      })
    },
    // 保存
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.ruleForm.id) {
            updateExamineItem(data).then(res => {
              if (res.code === '0') {
                this.returnBack()
              } else {
