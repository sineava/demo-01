...
import { pageList, deleteSituation } from '@/api/general-services/enterprise-self-inspection/examineSituation'
  ...
  methods: {
    ...
    handleDelete(index, item) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '操作确认').then(() => {
        deleteSituation(item.id).then(res => {
          if (res.code === '0') {
