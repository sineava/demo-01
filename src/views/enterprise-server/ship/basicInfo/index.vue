        ...
        >
          ...
          <el-descriptions-item label="经济类型:">
            <el-form-item v-if="isEdit" prop="economicType">
              <el-input v-model.trim="info.economicType" placeholder="请输入电子邮箱" />
            </el-form-item>
...
<script>
import { getEnShipManageInfo,updateBasicInfo } from '@/api/general-services/enterpriseInfo/enterpriseInfo'
...
export default {
  ...
  async mounted() {
    const res = await getEnShipManageInfo()
    this.info = res.data
  ...
  methods: {
    ...
    uploadPhotoSuccess(file) {
      this.info.enterpriseBusinessInfoVo.certificatePicturePath = file.response.data
      this.photoList.push({
    ...
    uploadPhotoRemove(file) {
      ...
      this.info.enterpriseBusinessInfoVo.certificatePicturePath = ''
    }
  }
}
</script>
