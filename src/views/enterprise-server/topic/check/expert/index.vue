      ...
      <el-row :gutter="20">
        ...
        <el-col :span="18">
          <el-card class="topic-card">
            邀请专家人数：<span>{{ count }} (人)</span>
          </el-card>
          <el-row :gutter="10">
            <el-col>
              <el-card class="card-m">
                <div slot="header" class="clearfix">
                  <span>安全检查邀请专家比例</span>
                </div>
                <Charts :option="inviteOpt" />
              </el-card>
            </el-col>
          </el-row>
        </el-col>
...
import { getTopicList, exportTopicList, getTopicStat} from '@/api/general-services/topic/safety-expert-statistical'
  ...
  data() {
    return {
      ...
      options:[
        ...
        {label:'专家名称',type:'text',isShow:true,prop:'expertName'},
        {label:'所属单位',type:'text',isShow:true,prop:'expertDepartmentName'},
        {label:'手机号码',type:'text',isShow:true,prop:'expertMobilePhone'},
        {label:'职务',type:'text',isShow:true,prop:'expertPost'},
        {label:'技术职称',type:'text',isShow:true,prop:'expertTechnicalTitle'},
        {label:'参与计划名称',type:'text',isShow:true,prop:'planningName'},
        {label:'计划类型',type:'text',isShow:true,prop:'typeName'},
        ...
        {label:'检查时间',type:'text',isShow:true,prop:'planningTime'},
      ],
      count: undefined,
      inviteOpt: {},
      queryData:{}
  ...
  methods: {
    ...
    topic(queryData){
      ...
      getTopicStat(this.queryData).then(res=>{
        if (res.code === '0') {
          if (!res.data){
            this.inviteOpt=new Pie().option
            this.count="0"
          } else {
            const {count, isExpertInvitedCount, planningCount} = res.data || {}
            this.count = count
            this.inviteOpt = new Pie([{ name: '邀请专家', value: isExpertInvitedCount }, { name: '未邀请专家', value: planningCount - isExpertInvitedCount }]).option
          }
        }
      })
    },
    ...
    getExportData(queryData,callback) {
      ...
      exportTopicList(queryData).then(res => {
        ...
        arr.unshift(header)
        callback({ data: [...arr], title: "邀请专家数据" })
      })
    }
  }
}
