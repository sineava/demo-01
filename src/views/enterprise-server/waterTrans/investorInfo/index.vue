...
import { pageWaterTransportEnList,deleteInvestors } from '@/api/water-transportation-services/information/investors'
...
export default {
  ...
  methods: {
    ...
    handleDetail(type,item) {
      if (type==='detail'){
        this.$router.push({ path: '/waterTrans/investorInfoDetail', query: { id: item.id }})
      } else {
        this.$router.push({ path: '/waterTrans/investorInfoEdit', query: { id: item.id || '' }})
      }
    }
  }
}
