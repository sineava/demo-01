...
import { pageWaterTransportEnList,deletePersonnel } from '@/api/water-transportation-services/information/manage-person'
...
export default {
  ...
  methods: {
    ...
    handleDetail(type,item) {
      if (type==='detail'){
        this.$router.push({ path: '/waterTrans/personnelInfoDetail', query: { id: item.id,oper:type }})
      } else {
        this.$router.push({ path: '/waterTrans/personnelInfoEdit', query: { id: item.id,oper:type }})
      }
      // this.$router.push({ path: '/waterTrans/personnelInfoDetail', query: { id: item.id }})
    }
  }
}
</script>
