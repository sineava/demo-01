...
import BasicsBuilding from '../components/basics-building'// 房屋
  ...
  methods: {
    ...
    submit(){
      ...
      this.tableData.forEach(item=>{
        data.push({structuresInfoDto:item.submit,typeInfoId:item.submit.typeId})
      })
    ...
    // 验证提交表单
    verSubmitFrom(data){
      const basics_data = basics(data,102)
      const finance_data = finance(data,102)
      const use_data = use(data,102)
      const others_data = others(data,102)
