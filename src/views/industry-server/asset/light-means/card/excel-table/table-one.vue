  ...
  methods: {
    ...
    submit(){
      ...
      this.tableData.forEach(item=>{
        data.push({housingInfoDto:item.submit,typeInfoId:item.submit.typeId})
      })
    ...
    // 验证提交表单
    verSubmitFrom(data){
      const basics_data = basics(data,1)
      const finance_data = finance(data,1)
      const use_data = use(data,1)
      const others_data = others(data,1)
    ...
    async queryAssetsTypeTree(){
      const res = await queryAssetsType({type:1})
    ...
    format(value,format){
      ...
      if (date!=='Invalid date'){
        return moment(new Date(date).getTime()+(24*3600*1000)).format(format || 'YYYY-MM-DD')
      }
      return ''
    },
    getValue(prop,val){
