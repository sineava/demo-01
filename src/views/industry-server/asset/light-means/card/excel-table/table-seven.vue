...
import BasicsSeven from '../components/basics-seven'// 房屋
  ...
  methods: {
    ...
    submit(){
      ...
      this.tableData.forEach(item=>{
        data.push({suppliesInfoDto:item.submit,typeInfoId:item.submit.typeId})
      })
    ...
    // 验证提交表单
    verSubmitFrom(data){
      const basics_data = basics(data,7)
      const finance_data = finance(data,7)
      const use_data = use(data,7)
      const others_data = others(data,7)
