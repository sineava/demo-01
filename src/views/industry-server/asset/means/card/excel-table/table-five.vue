...
import BasicsFive from '../components/basics-five'// 房屋
  ...
  methods: {
    ...
    submit(){
      ...
      this.tableData.forEach(item=>{
        data.push({furnitureInfoDto:item.submit,typeInfoId:item.submit.typeId})
      })
    ...
    // 验证提交表单
    verSubmitFrom(data){
      const basics_data = basics(data,5)
      const finance_data = finance(data,5)
      const use_data = use(data,5)
      const others_data = others(data,5)
    ...
    getValue(prop,val){
      ...
      case 'propertyRF':
        ...
        break
      case 'userThe':
        value = val// val==='本单位' ? 1 : 2
        break
      case 'useDeptId': // 使用部门ID
        value = this.useDeptId(val)
        break
      case 'deptId': // 管理部门ID
