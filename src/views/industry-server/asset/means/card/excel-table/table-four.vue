...
import BasicsFour from '../components/basics-four'// 房屋
  ...
  methods: {
    ...
    submit(){
      ...
      this.tableData.forEach(item=>{
        data.push({bookInfoDto:item.submit,typeInfoId:item.submit.typeId})
      })
    ...
    // 验证提交表单
    verSubmitFrom(data){
      const basics_data = basics(data,4)
      const finance_data = finance(data,4)
      const use_data = use(data,4)
      const others_data = others(data,4)
