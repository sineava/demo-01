...
import BasicsThree from '../components/basics-three'// 房屋
  ...
  methods: {
    ...
    submit(){
      ...
      this.tableData.forEach(item=>{
        data.push({culturalRelicsInfoDto:item.submit,typeInfoId:item.submit.typeId})
      })
    ...
    // 验证提交表单
    verSubmitFrom(data){
      const basics_data = basics(data,3)
      const finance_data = finance(data,3)
      const use_data = use(data,3)
      const others_data = others(data,3)
