...
import BasicsTwo from '../components/basics-two'//
  ...
  methods: {
    ...
    submit(){
      ...
      this.tableData.forEach(item=>{
        data.push({equipmentInfoDto:item.submit,typeInfoId:item.submit.typeId})
      })
    ...
    // 验证提交表单
    verSubmitFrom(data){
      const basics_data = basics(data,2)
      const finance_data = finance(data,2)
      const use_data = use(data,2)
      const others_data = others(data,2)
