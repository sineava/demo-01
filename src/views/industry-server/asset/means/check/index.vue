...
<script>
import { getList,delinventory } from '@/api/means/check'
...
export default {
  ...
  methods: {
    ...
    handleEdit(item,oper){
      this.$router.push({ path: '/means/check/details', query: { id: item.id ,oper: oper}})
    },
    handleInventory(item){
      this.$router.push({ path: '/means/check/inventory', query: { id: item.id}})
    },
    // 删除
    handleDelete(item){
