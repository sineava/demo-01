...
<script>
import { getList } from '@/api/credit/port/data-summary'
...
export default {
  ...
  methods: {
    ...
    reportRecord(){
      this.$router.push({ path: '/credit/port/data-summary/record', query: { orgCode:this.org.orgCode,orgName:this.org.orgName }})
    },
    /**
     * 切换每页大小
     */
    ...
    async getExportData(cb) {
      ...
      this.table.forEach(item=>{
        const obj = {
          ...
          state:getLabelName(item.documentTypeCode,this.certificateStatusOptions)
        }
        arr.push(obj)
      })
      arr.unshift(header)
      cb({ data: [...arr], title: '港口企业信用数据上报汇总' })
    }
  }
}
</script>
