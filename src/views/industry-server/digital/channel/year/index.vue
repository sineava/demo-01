<template>
  <el-card class="body-card">
    ...
    >
      ...
      <el-table-column label="下年计划" :show-overflow-tooltip="true">
        <template v-slot="scope">
          <a
            v-for="(file, index) in scope.row.nextWorkPlanFile ? JSON.parse(scope.row.nextWorkPlanFile) : []"
            :key="index"
            class="download"
...
import { getYearList, auditInfo } from '@/api/channel-services/channel/report-conclusion'
export default {
  components: {
    pagination
  },
  ...
  methods: {
    ...
    fetchData() {
      ...
      getYearList({
        ...this.searchForm,
        timeInfo: momentDate(this.searchForm.timeInfo, 'YYYY')
      }).then(res => {
    ...
    handleEdit(type, item) {
      this.$router.push({ path: '/digital/channel/yearEdit', query: { type, reportBasicId: item.reportBasicId, timeInfo: item.timeInfo }})
    },
    /**
     * 航道规范
     */
    handleStandard() {
      // !type 2月报 3年报
      this.$router.push({ path: '/digital/channel/claim', query: { type: 3 }})
    },
    // 退回
    handleReback(item) {
      ...
      this.$nextTick(() => {
        ...
        this.sureForm.id = item.reportBasicId
      })
    },
    // 确认退回
    handleSure() {
