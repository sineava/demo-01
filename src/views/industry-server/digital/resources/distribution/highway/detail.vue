  ...
  <div class="base-ruleForm">
    ...
    >
      <el-form-item
        ...
        prop="name"
      >
        <el-input v-model.trim="ruleForm.name" maxlength="100" placeholder="请输入公路名称" />
      </el-form-item>
      <el-form-item
        label="起止点:"
        prop="startingEndingPoint"
      >
        <el-input v-model.trim="ruleForm.startingEndingPoint" maxlength="100" placeholder="请输入起止点" />
      </el-form-item>
      <el-form-item
        label="总里程(米):"
        prop="totalMileage"
      ...
      >
        <el-input v-model.trim="ruleForm.trafficDirection" maxlength="100" placeholder="请输入通行方向" />
      </el-form-item>
      <el-form-item
        label="限速限高(具体位置或线段):"
        prop="speedLimit"
...
<script>
import { getInfo, addData, updateData } from '@/api/data-management/overall-distribution/highway'
export default {
  data() {
    return {
      isEdit: false,
      ...
      rules: {
        name: [
          { required: true, message: '请输入公路名称', trigger: 'blur' }
        ],
        startingEndingPoint:[
          { required: true, message: '请输入起止点', trigger: 'blur' }
        ],
        totalMileage:[
          { required: true, message: '请输入请输入总里程(米)', trigger: 'blur' }
        ]
      },
    }
  },
