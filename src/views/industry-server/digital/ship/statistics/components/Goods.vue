...
import { queryGoodsList } from '@/api/channel-services/ship/ship-type-statistical'
...
export default {
  ...
  methods: {
    ...
    async fetchData(name = '') {
      this.loading = true
      this.show = false
      const res = await selectLike({ name })
      if (res.code === '0') {
        ...
        const otherRes = await queryGoodsList({
          ...this.searchForm,
