    ...
    <div class="app-container">
      ...
      >
        ...
        <el-table-column
          ...
          label="操作"
          align="center"
          :width="(button_roles.desc ? 90 : 0) + (button_roles.update ? 90 : 0) + (button_roles.delete ? 90 : 0)"
        >
          <template v-slot="scope">
            <el-button
...
<script>
import { getPageList, deleteOrgan } from '@/api/emergency-services/organization'
...
export default {
  ...
  methods: {
    ...
    handleEdit(type, item) {
      this.$router.push({ path: '/emergency/data/agencyEdit', query: { id: item ? item.id : null, type }})
    },
    /**
     * 删除
     */
    handleDelete(_, item) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '操作确认').then(() => {
        deleteOrgan({ id: item.id }).then(res => {
          if (res.code === '0') {
