...
<script>
import { queryEnterpriseAnalysePage } from '@/api/port-services/port-enterprise-charge-management/charge-industry'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      queryEnterpriseAnalysePage(queryInfo).then(res => {
    ...
    handleDetail(index, item) {
      ...
      this.$router.push({ path: '/enterprisecharges/analysisDetail', query: { tableType: item.tableType, tabName: tabName, enterpriseId: item.enterpriseId, above: item.above, below: item.below }})
    }
  }
}
</script>
