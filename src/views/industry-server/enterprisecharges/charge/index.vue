...
<script>
import { queryEnterprisePage } from '@/api/port-services/port-enterprise-charge-management/charge-industry'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      queryEnterprisePage(queryInfo).then(res => {
    ...
    handleDetail(index, item) {
      this.$router.push({ path: '/enterprisecharges/chargeDetail', query: { tableType: item.tableType, enterpriseId: item.enterpriseId }})
    }
  }
}
</script>
