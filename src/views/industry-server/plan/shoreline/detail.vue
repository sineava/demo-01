    ...
    <div class="basic-detail">
      ...
      >
        ...
        <el-descriptions
          ...
          border
        >
          <el-descriptions-item label="批复文号:">
            {{ ruleForm.approvalNumber }}
          </el-descriptions-item>
          <el-descriptions-item label="批复时间:">
            {{ ruleForm.approvalDate }}
          </el-descriptions-item>
        </el-descriptions>
        <el-form-item
          label-width="0"
  ...
  methods: {
    ...
    async fetchDetail(id) {
      getDetail({ id }).then(res => {
        this.ruleForm = {
          ...res.data,
          businessFile: JSON.parse(res.data.businessFile),
          approvalFile: JSON.parse(res.data.approvalFile)
        }
      })
    },
    // 返回
    returnBack() {
      this.$router.push({ path: '/plan/shoreline' })
    }
  }
}
</script>
