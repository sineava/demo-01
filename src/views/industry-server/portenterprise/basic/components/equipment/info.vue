...
export default {
  ...
  mounted() {
    if (this.objData.id) {
      this.facilityName = this.objData.facilityName
      this.facilityCategory = this.objData.facilityCategory
      this.facilityId = this.objData.id
      this.fetchData()
    }
  },
  methods: {
     ...
     */
    handleDetail(item) {
      this.$emit('join', { name: '检测维护详情', data: { id: item.id, facilityId: this.facilityId, facilityName: this.facilityName, facilityCategory: this.facilityCategory }})
    }
  }
}
</script>
