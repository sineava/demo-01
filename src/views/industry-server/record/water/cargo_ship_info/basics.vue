    ...
    >
      ...
      >
        ...
        <el-descriptions-item label="备案状态:">
          <el-tag :type="['','warning','success','success'][ruleForm.isStatus-1]">{{ ['审核中','退回','审核通过','审核通过'][ruleForm.isStatus-1] }}</el-tag>
        </el-descriptions-item>
      </el-descriptions>
      ...
      >
        ...
        <el-descriptions-item :span="2">
          ...
          <el-form-item v-if="oper==='add' || oper==='edit'" prop="enterpriseId">
            <el-select
      ...
      >
        ...
        <el-descriptions-item :span="2">
          <template slot="label"><i v-if="oper==='add' || oper==='edit'" class="required" />船舶来源:</template>
          <el-form-item v-if="oper==='add' || oper==='edit'" prop="shipSource">
            <el-radio-group v-model="ruleForm.shipSource">
      ...
      <div class="icon-title">
        <span class="span" /> <i v-if="oper==='add' || oper==='edit'" class="required" style="margin-right: 10px;" /> 拟建船舶情况
        <span style="font-size: 12px; font-weight: 500; color: #ff2a2a;">（记得点击“保存”按钮哦！）</span>
        <el-button
      ...
      >
        <upload
          ...
          :limit="9"
          multiple
          fileAccept=".png,.jpeg,.jpg,.pdf"
          tips="仅可上传jpg、png、jpeg、pdf格式，上传文件大小不得超过20m"
...
import { addFilingData, updateFilingData,acceptFilingData,auditFilingData,getEnterpriseList,getFilingInfo } from '@/api/water-transportation-services/keep-on-record/ordinary-freighter-transport-filing'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        isDisclaimers:[
          { required: true, message: '请勾选申请承诺', trigger: 'change' }
        ]
      },
      loading: false,
      info: {}
  ...
  async mounted() {
    this.id = this.fid
    this.init()
    const res = await getEnterpriseList({type:3})
  ...
  methods: {
    ...
    async addSave() {
      ...
      for (let i=0;i<this.options.length;i++){
        if (this.options[i].id===this.ruleForm.enterpriseId){
          enterpriseName = this.options[i].enterpriseName
        }
      }
      let data = {
        id:this.ruleForm.id || '',
