<template>
  <el-card class="body-card">
    ...
    >
      ...
      <el-radio-group v-model="type" style="float: right; margin: 10px 0;" @change="changeType">
        <el-radio-button :label="1">备案查询</el-radio-button>
        <el-radio-button v-if="button_roles.handling" :label="2">业务受理</el-radio-button>
        <el-radio-button v-if="button_roles.center_audit" :label="3">业务审核</el-radio-button>
      </el-radio-group>
    </div>
    <div class="app-container">
      ...
      >
        ...
        />
        <el-table-column
          v-if="button_roles.handling || button_roles.receipt || button_roles.desc || button_roles.center_audit || button_roles.update || button_roles.delete"
          label="操作"
          align="center"
          :width="120"
        >
          <template v-slot="scope">
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item v-if="button_roles.handling" :disabled="scope.row.isStatus < 3 ? false :true " @click.native="handleDetail(scope.row, 'handling')">业务办理</el-dropdown-item>
                <el-dropdown-item v-if="button_roles.center_audit" :disabled="(scope.row.isStatus===3 && scope.row.isExamine===1) ? false :true " @click.native="handleDetail(scope.row, 'handling')">业务审核</el-dropdown-item>
...
export default {
  ...
  methods: {
     ...
     */
    handleAdd() {
      this.$router.push({ path: '/record/water/other/addCargoShip', query: { oper: 'add' }})
    },
    // 删除
    ...
    handleDetail(item, isEdit) {
      let oper = isEdit
      if (isEdit==='handling'){
        if (item.isStatus===3 && item.isExamine===1){
          oper = 'audit'
        } else {
          oper = 'accept'
        }
      }
      this.$router.push({ path: '/record/water/other/addCargoShip', query: { id: item.id, oper: oper }})
    },
    async download(item){
      ...
      info.data.shipVoList.map((item,index)=>{
        ...
        item.totalLength = item.totalLength || ''
        item.totalWidth = item.totalWidth || ''
        item.shipName = item.shipName || ''
        item.investmentAmount = item.investmentAmount || ''
        item.buildingShipyard = item.buildingShipyard || ''
        return item
      })
      const wordData ={
