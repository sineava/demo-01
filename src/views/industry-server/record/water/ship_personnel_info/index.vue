      ...
      >
        ...
        >
          <template v-slot="scope">
            ...
            <el-button
              v-if="scope.row.isStatus===1 && button_roles.handling"
              size="mini"
              type="primary"
              plain
...
import { pageHySmPersonFilingList } from '@/api/water-transportation-services/keep-on-record/ship-manage-person-filing'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      pageHySmPersonFilingList(data).then(res => {
    ...
    handleDetail(item, isEdit) {
      this.$router.push({ path: '/record/water/ship/personnelInfoDetail', query: { id: item.id, isEdit }})
    }
  }
}
</script>
