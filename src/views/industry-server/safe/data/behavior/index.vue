...
<script>
import { pageList, deleteViolation } from '@/api/general-services/safety-supervision-and-inspection/safetyViolation'
...
export default {
  ...
  methods: {
    ...
    handleAdd(item, disabled) {
      this.$router.push({ path: '/safe/data/behaviorAdd', query: { item: item, disabled: disabled }})
    },
    /**
     * 删除
     */
    handleDelete(item) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '操作确认').then(() => {
        deleteViolation(item.id).then(res => {
          if (res.code === '0') {
