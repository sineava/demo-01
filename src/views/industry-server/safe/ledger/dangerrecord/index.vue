    ...
    <div class="app-container">
      ...
      >
        ...
        <el-table-column label="危险等级" :show-overflow-tooltip="true">
          <template v-slot="scope">
            <span v-if="scope.row.riskLevel"><el-tag :type="scope.row.riskLevel === 2 ? 'danger' : 'warning'">{{ scope.row.riskLevel===2 ? '重大' : '一般' }}</el-tag></span>
          </template>
        </el-table-column>
        <el-table-column label="是否整改" :show-overflow-tooltip="true">
          <template v-slot="scope">
            <el-tag :type="scope.row.isStatus ? 'success' : 'danger'">{{ scope.row.isStatus ? '是' : '否' }}</el-tag>
          </template>
        </el-table-column>
...
<script>
import { getHyList } from '@/api/general-services/enterprise-self-ledger/dangerrecord'
...
export default {
  ...
  methods: {
    ...
    handleDetail(item, disabled) {
      this.$router.push({ path: '/safe/ledger/dangerrecordDetail', query: { id: item.id, disabled: disabled }})
    },
    /**
     * 重置查询条件
     */
