    ...
    <div class="app-container">
      ...
      >
        ...
        <el-table-column
          ...
          prop="targetName"
        />
        <el-table-column label="已完成整改" :show-overflow-tooltip="true">
          <template v-slot="scope">
            <span>{{ scope.row.isComplete === 1 ? '是' : '否' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="已完成复查" :show-overflow-tooltip="true">
          <template v-slot="scope">
        ...
        >
          <template v-slot="scope">
            <el-button
              ...
              size="mini"
              plain
              :disabled="scope.row.organizationId === userInfo.orgid || userInfo.orgcategory === '1'  ? false : true"
              @click="handleDetail(scope.row)"
            >
              <em class="el-icon-plus" /> 填写复查记录
            </el-button>
            <el-button
...
<script>
import { hyPageList } from '@/api/general-services/safety-supervision-and-inspection/reviewRecord'
