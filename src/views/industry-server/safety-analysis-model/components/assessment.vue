  ...
  >
    ...
    >
      <el-descriptions-item>
        <template slot="label"><i v-if="isEdit" class="required" />评估区域</template>
        <el-form-item v-if="isEdit" prop="riskAreaId">
          <el-select
            v-model="ruleForm.riskAreaId"
            clearable
            placeholder="请选择评估地区"
            @change="change"
          >
        ...
        <span v-else>{{ ruleForm.riskAreaName }}</span>
      </el-descriptions-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        riskAreaId:[
          { required: true, message: '请选择评估地区', trigger: 'change' }
        ],
        appraisalQuarter:[
  ...
  methods: {
    change(val){
      for (let i = 0;i<this.areaList.length;i++){
        if (val===this.areaList[i].id){
          this.ruleForm.riskAreaName = this.areaList[i].areaName
          return true
        }
      }
    },
    initData(data){
      return {
        ...
        riskAreaId:data?.riskAreaId, // 区域ID
        
        riskAreaName:data?.riskAreaName,// 区域名称
