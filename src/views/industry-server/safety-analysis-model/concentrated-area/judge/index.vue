...
import { initData, assessData as assessInfo, completeData,areaInfo } from '@/api/analysis-model/centralized-area'
...
export default {
  ...
  methods: {
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/concentrated-area/analysis'})
    },
    changeRoute(){
      this.init()
    ...
    async assess() {
      ...
      this.assessData = (await startAssess(3)).data
      this.active = 2
    },
    // 提交评估
    async submitAssess() {
