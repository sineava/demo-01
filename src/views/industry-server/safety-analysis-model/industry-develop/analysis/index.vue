...
import { currentData, riskData, trendData, getList ,del } from '@/api/analysis-model/level-index-area'
...
import { riskColor } from '@/utils/config'
...
export default {
  ...
  methods: {
    getLevelNameToColor,
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/industry-develop/judge'})
    },
    // 查询风险等级列表
    async fetchLevel() {
      this.levels = (await getLevel(4)).data
    },
    async fetchFirmList() {
      // 辖区水上交通安全形式分析
      this.zoneList = (await getZoneList(4)).data || []
    ...
    handleDetail(item,isDetail) {
      this.$router.push({ path: '/safety-analysis-model/industry-develop/judge', query: { id: item.id ,isDetail:isDetail}})
    },
    /**
     * 获取全部风险评估信息
     */
