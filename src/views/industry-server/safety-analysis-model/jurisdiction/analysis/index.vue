...
import { currentData, riskData, trendData, getList, delAppraisalArea } from '@/api/analysis-model/safe-mode-appraisal-area'
...
export default {
  ...
  methods: {
    getLevelNameToColor,
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/jurisdiction/judge'})
    },
    // 查询风险等级列表
    async fetchLevel() {
      this.levels = (await getLevel(1)).data
    },
    async fetchZoneList() {
      // 辖区水上交通安全形式分析
      this.zoneList = (await getZoneList(1)).data || []
    ...
    handleDetail(item,isDetail) {
      this.$router.push({ path: '/safety-analysis-model/jurisdiction/judge', query: { id: item.id ,isDetail:isDetail}})
    },
    /**
     * 删除
     */
    handleDelete(item){
      delAppraisalArea(item.id).then(res=>{
        if (res.code==='0'){
