...
import { initData, assessData as assessInfo, completeData , areaInfo } from '@/api/analysis-model/safe-mode-appraisal-area'
...
export default {
  ...
  methods: {
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/jurisdiction/analysis'})
    },
    changeRoute(){
      this.init()
    ...
    async assess() {
      ...
      this.assessData = (await startAssess(1)).data
      this.active = 2
    },
    // 提交评估
    async submitAssess() {
