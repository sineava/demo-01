    ...
    <div class="body-container">
      ...
      <div v-if="show" class="content">
        ...
        <el-card style="margin: 10px 0;">
          ...
          <Charts style="height: 360px;" :option="lineOption" />
        </el-card>
...
import { currentData, riskData, trendData, getList, del } from '@/api/analysis-model/risk-analysis-area'
...
export default {
  ...
  methods: {
    getLevelNameToColor,
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/water-transport/judge'})
    },
    // 查询风险等级列表
    async fetchLevel() {
      this.levels = (await getLevel(2)).data
    },
    async fetchZoneList() {
      // 辖区水上交通安全形式分析
      this.zoneList = (await getZoneList(2)).data || []
    ...
    handleDetail(item,isDetail) {
      this.$router.push({ path: '/safety-analysis-model/water-transport/judge', query: { id: item.id ,isDetail:isDetail}})
    },
    /**
     * 获取全部风险评估信息
     */
