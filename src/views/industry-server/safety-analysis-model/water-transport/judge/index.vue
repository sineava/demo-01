...
import { initData, assessData as assessInfo, completeData ,areaInfo } from '@/api/analysis-model/risk-analysis-area'
...
export default {
  ...
  methods: {
    changeType(e){
      this.$router.push({ path: '/safety-analysis-model/water-transport/analysis'})
    },
    changeRoute(){
      this.init()
    ...
    async assess() {
      ...
      this.assessData = (await startAssess(2)).data
      this.active = 2
    },
    // 提交评估
    async submitAssess() {
