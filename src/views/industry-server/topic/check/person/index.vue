...
import { getTopicList, exportTopicList, getTopicStat } from '@/api/general-services/topic/safety-inspector-statistical'
  ...
  data() {
    return {
      ...
      options:[
        ...
        {label:'检查人员',type:'text',isShow:true,prop:'name'},
        {label:'所属单位',type:'text',isShow:true,prop:'departmentName'},
        {label:'手机号码',type:'text',isShow:true,prop:'mobilePhone'},
        {label:'职务',type:'boolean-only-num',isShow:true,prop:'post',payload:[
          {value: 0, label: '局长'},
          {value: 1, label: '副局长'},
          {value: 2, label: '处长'},
          {value: 3, label: '副处长'},
  ...
  methods: {
    ...
    topic(queryData){
      ...
      getTopicStat(this.queryData).then(res=>{
        if (res.code === '0') {
          const {count="0", departmentTypeStatisticalList = []} = res.data || {}
          this.count = count
          let eName = []
          let eValue = []
    ...
    getExportData(queryData,callback) {
      ...
      exportTopicList(queryData).then(res => {
        ...
        arr.unshift(header)
        callback({ data: [...arr], title: "检查人员数据" })
      })
    }
  }
}
