...
import { getInCondition } from '@/api/port-services/construction-statistics/construction-statistics'
import { queryTopicPageList, exportTopicList } from '@/api/port-services/construction-dangerous-goods/safety-condition-review'
...
export default {
  ...
  data() {
    return {
      ...
      searchData:[
        ...
        {label:'工程开始时间范围',prop:['screenStartTime','screenEndTime'],form:'daterange',data:'',format: ['yyyy-MM-dd 00:00:00','yyyy-MM-dd 23:59:59']}
      ],
      options:[
        {label:'序号',type:'index',isShow:true },
        {label:'项目名称',type:'text',isShow:true,prop:'projectName'},
        {label:'项目类型',type:'text',isShow:true,prop:'projectType'},
        {label:'所在水域',type:'text',isShow:true,prop:'inWaters'},
        {label:'工程地址',type:'text',isShow:true,prop:'projectAddress'},
        {label:'工程开始时间',type:'text',isShow:true,prop:'startTime'},
        {label:'工程结束时间',type:'text',isShow:true,prop:'endTime'},
        {label:'申请事项',type:'text',isShow:true,prop:'applyItems'},
        {label:'评估机构名称',type:'text',isShow:true,prop:'assessmentAgencyName'},
        {label:'建设规模',type:'text',isShow:true,prop:'constructionScale'},
        {label:'联系人',type:'text',isShow:true,prop:'contact'},
        {label:'装卸储主要存危险货物名称',type:'text',isShow:true,prop:'dangerousGoodsName'},
        ...
        {label:'工程开始时间范围',type:'text',isShow:true,prop:'applyTime'},
        {label:'申请原因',type:'text',isShow:true,prop:'applyReason'},
        {label:'储罐容量',type:'text',isShow:true,prop:'tankCapacity'},
        {label:'单位地址',type:'text',isShow:true,prop:'unitAddress'},
        {label:'港口建设项目概况',type:'text',isShow:true,prop:'overview'}
      ],
      deptOpt: {}, // *所在港区分类统计
      timeOpt: {}, // *时间趋势
      reviewResult: [
  ...
  methods: {
    ...
    async fetchData(queryData) {
      ...
      this.loading = true
      queryTopicPageList(this.queryData).then(res=>{
        this.loading = false
        if (res.code === '0') {
          this.loading = false
    ...
    topic(queryData){
      ...
      getInCondition(this.queryData).then(res=>{
        if (res.code === '0') {
          ...
          this.deptOpt = new Bar(eName, eValue,{top: '30px',left: '20px',right: '65px',bottom: eName.length>25 ? '40px' : '30px',containLabel: true},eName.length>10?90:0,'数量','机构').option
        }
      })
    },
    ...
    getExportData(queryData,callback) {
      ...
      exportTopicList(queryData).then(res => {
        ...
        arr.unshift(header)
        callback({ data: [...arr], title: "安全条件审查" })
      })
    }
  }
}
