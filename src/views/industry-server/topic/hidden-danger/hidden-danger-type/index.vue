...
export default {
  ...
  data() {
    return {
      ...
      options:[
        ...
        {label:'检查项分类',type:'text',isShow:true,prop:'itemCategoryName'},
  ...
  methods: {
    ...
    async fetchData(queryData) {
      ...
      this.loading = true
      getHdTopicList(queryData).then(res=>{
        this.loading = false
        if (res.code === '0') {
          this.loading = false
    ...
    topic(queryData){
      ...
      getHdTopicStat(queryData).then(res=>{
        this.loading = false
        if (res.code === "0") {
          const {
