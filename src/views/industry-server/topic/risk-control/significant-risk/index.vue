      ...
      <el-row :gutter="20">
        ...
        <el-col :span="18">
          <el-card class="topic-card">
            ...
            <span style="margin-left: 20%">参与评估企业数量：<span>{{ info.enterpriseCount || 0 }} (个)</span></span>
            <span style="margin-left: 20%">已完成登记企业数量：<span>{{ info.completeCount || 0 }} (个)</span></span>
          </el-card>
          <el-row :gutter="10">
            <el-col :span="8">
...
import { getTopicList, exportTopicList, getTopicStat } from '@/api/port-services/topic/assess-statistics'
  ...
  methods: {
    ...
    topic(queryData){
      ...
      getTopicStat(this.queryData).then(res=>{
        if (res.code === '0') {
          const { count="0", completeCount="0", enterpriseCount="0", factorsTypeList = [], isPerfectList = [], levelTypeStatisticsList = [], timeStatisticsList = [] } = res.data || {}
          this.info = {
            count,
            completeCount,
            enterpriseCount
          }
          this.levelOpt = new Pie(levelTypeStatisticsList.map(x => ({ name: x.type && this.riskGrade.find(y => y.value === x.type).label, value: x.count }))).option
          this.factorOpt = new Pie(factorsTypeList.map(x => ({ name: x.type && ['人的因数','设备因素','环境因素','管理因素'][x.type - 1], value: x.count }))).option
          this.rectifyOpt = new Pie(isPerfectList.map(x => ({ name: (x.type || x.type === 0) && ['未完成整改','已完成整改'][x.type], value: x.count }))).option
    ...
    getExportData(queryData,callback) {
      ...
      exportTopicList(queryData).then(res => {
        ...
        arr.unshift(header)
        callback({ data: [...arr], title: "重大风险数据" })
      })
    }
  }
}
